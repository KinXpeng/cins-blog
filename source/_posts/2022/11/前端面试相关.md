...
- 方法
  ...
  - ```html
    ...
    /**css**/ .parent { position: relative; } .child { position: absolute; top:
    0; bottom: 0; left: 0; right: 0; margin: auto; }
    ```

  - ```html
    .parent { position: relative; } .child { position: absolute; left: 50%; top:
...
优化方案，建议从 tcp 请求到接口请求到页面的绘制

##### 问题

vue 中改变 id=1 的数据到页面刷新都经历了什么过程

vue 中的 next tick

promise 的.then()是同步的还是异步的
